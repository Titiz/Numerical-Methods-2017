...
"""
    ...
    while (b-a) > tol:
        ...
        print("a:", Decimal(a))
        print("b      :", Decimal(b))
        print("(a+b)/2:", Decimal((a+b)/2))
        print("(a+b)/2:", (a+b)/2)
        print("(a+b)/2:",(Decimal(a) + Decimal(b))/Decimal(2.0))
        print("a:", Decimal(a))
        print("a+b = ", Decimal(a+b))
        midpoint = (a+b)/2.
        sfm = sign(f(midpoint))
        print("f(midpoint) = " +  repr(f(midpoint)))
        s = bitstring.pack('>d', a)
        sbit, wbits, pbits = s[:1], s[1:12], s[12:]
        s2 = bitstring.pack('>d', b)
        sbit2, wbits2, pbits2 = s2[:1], s2[1:12], s2[12:]
